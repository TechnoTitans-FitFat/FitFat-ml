...
def get_or_train_model(columns):
    ...
    if columns_tuple in models_cache:
        print(f"Use an existing model for columns: {columns}")
...
def get_recommendations_with_allergy_priority(user_new, valid_columns, data_new, meal_data, top_k=5):
    ...
    if has_allergy:
        ...
        print(f"User has allergy constraint: {user_allergy_encoded}")
        ...
        print(f"Found {len(safe_data)} safe meals for user's allergy")
        ...
        if len(safe_data) > 0:
            ...
            for idx in top_indices:
                original_idx = safe_meal_data.iloc[idx].name
                meal_id = str(safe_meal_data.iloc[idx]["_id"])
                similarity_score = similarities[idx]
                recommendations.append({
                    "meal_id": meal_id,
                    "similarity_score": float(similarity_score),
                    "is_allergy_safe": True,
    ...
    for idx in top_indices:
        ...
        if has_allergy:
            ...
            user_allergy = user_new['allergy_encoded'].iloc[0]
            is_safe = (meal_allergy == user_allergy)

        recommendations.append({
            "meal_id": meal_id,
            "similarity_score": float(similarity_score),
            "is_allergy_safe": is_safe,
            "original_index": idx
        })
...
def recommend_meal():
  ...
  if not valid_columns:
      ...
      return jsonify({"error": "No suitable recommendation could be found based on the available data"})
